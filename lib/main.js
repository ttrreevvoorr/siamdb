...
class Collection {
    ...
    find(where = {}) {
        ...
        const _evaluateOperator = (op, fieldValue, queryValue) => {
            switch (op) {
                case "$gt":
        ...
        const _evaluateCondition = (fieldValue, queryValue) => {
        ...
        if (where.id) {
            ...
            for (const key in this.documents) {
                ...
                const orConditions = where.$or || [where];
                const andConditions = where.$and || [];
                let docFound = orConditions.some((orCondition) => {
                    for (const query in orCondition) {
                        if (!_evaluateCondition(document.content[query], orCondition[query])) {
                            return false;
                        }
                    }
                    return true;
                ...
                if (docFound) {
                    for (const andCondition of andConditions) {
                        for (const query in andCondition) {
                            if (!_evaluateCondition(document.content[query], andCondition[query])) {
                                docFound = false;
                                break;
                            }
                        }
